...
pfile = '/22-12-19/participants.tsv' # Replace with actual folder name

# file from ADNI study data containing information about diagnosis per session
dfile = '/22-12-19/StudyData/DXSUM_PDXCONV_ADNIALL.csv' # Replace with actual folder name



###### Functions

def samplepath(data_path, pfile, dfile):
    """
    Function that prepares list of path to images (samples) and list of diagnoses (labels)
    
    Arguments: 
    - path to subject folders where images are stored
    - path to participant.tsv file generated by Clinica adni-to-bids pipeline
    - path to file containing diagnosis per session (DXSUM_PDXCONV_ADNIALL.csv from ADNI)
    
    Returns:
    list of image paths
    separate lists for image paths of classessses cn, ad and mci
    list of tuples (subject, session)
    separate list for subjects and sessions
    lists of diagnosis labels corresponding to images - one with letters and one with numbers
    list of excluded due to missing diagnosis
    list with sex information
    list with age information
    list with educational level
    list with mmse results
    ...
    diaglist = []
    dlist = []
    phase = 'ADNI3' # to enable restriction to a single study phase
    
    # Create dataframes for participants-file and diagnosis file
    p_df = pd.read_csv(pfile, sep= '\t', header=0)
    d_df = pd.read_csv(dfile, header=0)

    for path, subdirs, files in os.walk(data_path):
        subdirs.sort() # useful for debugging to have in alphabetical/numerical order
        for name in files:
            # ignore affine matrix file
            #if name.endswith('.mat'):
            
            # ignore other file types than the processed file
            if ('MNI152NLin2009cSym_desc-Crop' not in name):
                pass
            # restrict to processed baseline (M00) image
            elif ('MNI152NLin2009cSym_desc-Crop' in name) & (name[21:24] == 'M00'):
                # Extract participant_id in ADNI format from Clinica participants.tsv-file
                ptid = p_df[p_df['participant_id'] == name[0:16]].iloc[0]['alternative_id_1']
                # Convert session name from file name to same format as in diagnosis file
                if name[21:24] == 'M00':
                    if d_df['PTID'].value_counts()[ptid]==1:
                        pses = 'sc'
                    else:
                        pses = 'bl'
                        
                
                # Ensure that there is assigned diagnosis before loading image
                dcheck = d_df[(d_df['Phase']==phase)&(d_df['PTID']==ptid)&(d_df['VISCODE2']==pses)].iloc[0]['DIAGNOSIS']
                if dcheck in[1]:
                    img_path.append(path+ '/' + name)
                    cn_path.append(path+ '/' + name)
                    
                    sex = p_df[p_df['participant_id'] == name[0:16]].iloc[0]['sex']
                    sxlist.append(sex)
                    
                    age = p_df[p_df['participant_id'] == name[0:16]].iloc[0]['age_bl']
                    if (age<65):
                        age = 'age1'
                    ...
                    edu = p_df[p_df['participant_id'] == name[0:16]].iloc[0]['education_level']
                    if (edu<16):
                        edu = 'low' #(or low/medium)
                    elif (edu>15):
                        edu = 'high'
                    else:
                        pass
                    ...
                    mmse = p_df[p_df['participant_id'] == name[0:16]].iloc[0]['mmse_bl']
                    if (mmse<24):
                        mmse = 'low'
                    elif (mmse>23):
                        mmse = 'high'
                    ...
                    ethn = p_df[p_df['participant_id'] == name[0:16]].iloc[0]['ethnic_cat']
                    ethnlist.append(ethn)
                    
                    ### Add to list of tuples (subject, session)
                    subj_ses_list.append((name[0:16],name[21:24]))
                    subjlist.append(name[0:16])
                    sesslist.append(name[21:24])

                    ### get diagnosis
                    # Use participant id and corresponding session number to get diagnosis                
                    dval = d_df[(d_df['Phase']==phase)&(d_df['PTID'] == ptid)&(d_df['VISCODE2']==pses)].iloc[0]['DIAGNOSIS']
                    diagnoses.append(dval)
                 
                elif dcheck in [3]:
                    img_path.append(path+ '/' + name)
                    ad_path.append(path+ '/' + name)
                    ...
                    excluded.append([name[0:16], name[21:24], 'nan'])
                    
            # If session is not M00, replace uppercase letters with lowercase
            # Can be used later if expanding to include follow-up sessions
            elif ('MNI152NLin2009cSym_desc-Crop' in name) & (name[21:24] != 'M00'):
                sescap = name[21:24]
                pses = sescap.lower()

    
    # transform diagnoses from 1 = CN, 2 = MCI, 3 = Dementia (ADNI codes)
    # to 0 = CN, 1 = MCI, 2 = AD (dementia)
    for i in range(len(diagnoses)):
        if (diagnoses[i] == 1):
            diaglist.append(int(0))  # 2024-03-12: Changed to integer to see if necessary for sparse categorical crossentropy loss
            dlist.append('CN')
        elif (diagnoses[i] == 3):
            diaglist.append(int(2))
            dlist.append('AD')
        elif (diagnoses[i] == 2):
            diaglist.append(int(1))
            dlist.append('MCI')
        else:
            pass
    
    # Dataframe for subject data relevant for stratification and similar
    # use dropna() to remove subjects with missing age, or other rows with missing value
    strat = pd.DataFrame(list(zip(subjlist, dlist, diaglist, sxlist, agelist, edulist, mmselist, ethnlist, img_path)), columns = ['subj','diag_let','diag_num','sex', 'age', 'edu', 'mmse', 'ethn', 'imagepath']).dropna()
...
## Reference
## https://github.com/moboehle/Pytorch-LRP/blob/master/nitorch/transforms.py
...
def Flip(image, prob=0.5):
    ...
    Arguments:
        ...
        prob: probability to flip the image. Executes always when set to
             1. Default is 0.5
    """
    axis = 0 #(sagittal=0, coronal=1, axial=2)
    prob = prob
    ...
    if rand <= prob:
        augmented = np.flip(image, axis=axis).copy()
    else:
        augmented = image
...
def Translate(image, dist=(-3,3)):
    ...
    if isinstance(dist, tuple) or isinstance(dist, list):
        assert(len(dist) == 2)
        min_trans = np.min(dist)
        max_trans = np.max(dist)
    else:
        min_trans = -int(abs(dist))
        max_trans = int(abs(dist))
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, list_paths, labels, batch_size=8, dim=(imlength,imwidth,imheight), n_channels=1,
                 n_classes=3, shuffle=False, dist=(-3,3), prob=0.5):
        'Initialization'
        self.dim = dim
    ...
    def __data_generation(self, list_paths_temp):
        ...
        for i, paths in enumerate(list_paths_temp):
            ...
            if self.prob != None: 
                T1_img_data=Flip(T1_img_data,self.prob) #  Sagittal flip
            else:
                pass
            if self.dist != None:
                T1_img_data=Translate(T1_img_data,self.dist) # Sagittal translate
            else:
                pass
...
imagepath, cn_path, ad_path, mci_path, subjsession, subjlist, sesslist, diaglist, dlist, excluded, sxlist, agelist, edulist, mmselist, ethnlist, strat = samplepath(data_path, pfile, dfile)

print("length strat: ", len(strat))
print("length of strat['imagepath']: ",len(strat['imagepath']))

strat.to_csv('/MultiClassifier/stratlist_'+outputinfo+'.csv') # Replace with actual file location

###### Label dictionaries

# string coded, i.e. 'CN', 'MCI','AD'
dlbls = {}
keys = strat['imagepath'].tolist()
values = strat['diag_let'].tolist()
for i in range(len(strat['imagepath'])):
	dlbls[keys[i]] = values[i]

# number coded, i.e. 0,1,2
labels = {}
keys = strat['imagepath'].tolist()
values = strat['diag_num'].tolist()
for i in range(len(strat['imagepath'])):
...
print(strat['diag_let'].value_counts())
...
print("x_train_test stratification groups:")
targ = strat.diag_let + '_' + strat.sex + '_' + strat.age 
...
x_train_test, x_hold, y_train_test, y_hold = train_test_split(strat['imagepath'], strat['diag_let'], test_size=0.20, random_state=rseed, stratify=targ)
...
#Include arguments for data augmentation to ensure augmentation not used for validation data
...
#Include arguments for data augmentations
trainparams = {'dim': (imlength,imwidth,imheight),
         'batch_size': batch_size,
         'n_classes': 3,
         'n_channels': 1,
         'shuffle': False,
...
kf = KFold(n_splits=num_folds, random_state=rseed, shuffle=True) 


fold_no = 1

inp = np.array(x_train_test)
print("length of inp: ", len(inp))


for trainidx, testidx in kf.split(inp):	
	traindata = inp[trainidx] 
	testdata = inp[testidx]
	print('length traindata: ', len(traindata))
	print('length testdata: ', len(testdata))
	
	y_dummy = np.zeros(traindata.shape)
	
	
	# Statistics
	stats(traindata.tolist(), fold_no, 'xtrain_', dlbls)
	stats(testdata.tolist(), fold_no, 'xtest_', dlbls)
	
	# Divide into data sets with image paths
	partition = {'train':traindata, 'test':testdata}
	
	# Create generators
	train_generator = DataGenerator(partition['train'], labels, **trainparams)
	...
	# Callbacks
	path_checkpoint = '/MultiClassifier/Checkpoint/'+outputinfo+'_fold'+str(fold_no)+'_best_model.hdf5' # Replace with actual file location
	directory_checkpoint = os.path.dirname(path_checkpoint)
	model_checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath=path_checkpoint, monitor='val_loss', verbose=1,save_best_only=True)
	early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=patience)
	callbacks = [early_stopping, model_checkpoint]
	
	print("model.fit, fold no ", fold_no, ": ")
	# Fit data to model
	history = model.fit(
	train_generator,
	...
	print('path best model: ', path_checkpoint)
	
	# list all data in history
	print('History keys: ')
	print(history.history.keys())
	
	print("model.evaluate, fold no ", fold_no, ": ") 
	# #### Evaluate the model
	results = model.evaluate(test_generator, verbose=2)
	print("test loss: ",results[0])
	print("test acc: ",results[1])
	
	print("model.predict, fold no ", fold_no, ": ")
	# #### Model prediction
	...
	for i in range(len(prediction)):
		#predprob.append(list(prediction[i].round(3)))
		predprob.append(list(prediction[i]))
		
	print("predprob fold no ", fold_no, ":")
	print(predprob)
	...
	for i in range (len(test_generator)):
		for j in range(batch_size):
			true.append(test_generator[i][1][j])
			count1+=1
		count2+=1
	
	#true = np.array(true)
	print("true fold no ",fold_no, ":")
	print(true)
	
	predlabel = list(np.argmax(predprob, axis=1)) 
	print("predlabel fold no ", fold_no, ":")
	print(predlabel)					
	
	# List of internal validation subjects from testdata
	
	x_val = testdata.tolist()
	x_filt_val = []
	for i in range(len(x_val)):
		pos = x_val[i].find('_ses-')
		x_filt_val.append(x_val[i][(pos-16):pos])
	print('length x_filt_val: ', len(x_filt_val))
	strat_val = strat[(strat['subj'].isin(x_filt_val))]

	print("length of diagnosis list validation set: ", len(strat_val[['diag_let']]))
	print("No of instances per class validation set: ", pd.DataFrame(strat_val[['diag_let']]).value_counts(dropna=False))
	...
	for i in range(len(x_filt_val)):
		subject.append(x_filt_val[i])
		val_labels.append(strat_val[strat_val['subj']==x_filt_val[i]].iloc[0]['diag_let'])
		sx.append(strat_val[strat_val['subj']==x_filt_val[i]].iloc[0]['sex'])
		education.append(strat_val[strat_val['subj']==x_filt_val[i]].iloc[0]['edu'])
		mmse.append(strat_val[strat_val['subj']==x_filt_val[i]].iloc[0]['mmse'])
		ethn.append(strat_val[strat_val['subj']==x_filt_val[i]].iloc[0]['ethn'])
		agegroup.append(strat_val[strat_val['subj']==x_filt_val[i]].iloc[0]['age'])

	stratcv = pd.DataFrame(list(zip(subject, val_labels, sx, agegroup, education, mmse, ethn, predlabel, true, predprob)), columns = ['subj','diag','sex', 'age', 'edu', 'mmse', 'ethn', 'pred', 'true', 'prob'])
	print("length of stratcv: ", len(stratcv))
	stratcv.to_csv('/MultiClassifier/Statistics/Strattest/'+outputinfo+'_fold'+str(fold_no)+'stratcv.csv') # Replace With actual file location


	# # Global evaluation metrics
	
	print("Global evaluation metrics, fold no:", fold_no)
	cm = confusion_matrix(true, predlabel)
	print("confusion matrix global test set: ")
	print(cm)
	dispcm = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=target_names)
	dispcm.plot()
	dispcm.figure_.savefig('/MultiClassifier/Evaluation/'+outputinfo+'_fold'+str(fold_no)+'_globalCM.png') # Replace with actual file location
	
	try:
		roc_auc_score(true, predprob, multi_class='ovr')
	
		g_acc,g_bal_acc,g_recall_n,g_recall_w,g_spec_n,g_spec_w,g_prec_n,g_prec_w,g_F1_n,g_F1_w,g_MCC = evalmetric(true, predlabel, metrics)
	
		g_rocauc_ovr = roc_auc_score(true, predprob, multi_class='ovr')
		g_rocauc_ovo = roc_auc_score(true, predprob, multi_class='ovo')
	...
	print(female_cm)
	dispcm = ConfusionMatrixDisplay(confusion_matrix=female_cm, display_labels=target_names)
	dispcm.plot()
	dispcm.figure_.savefig('/MultiClassifier/Evaluation/'+outputinfo+'_fold'+str(fold_no)+'_femaleCM.png') # Replace with actual file location
	...
	print(male_cm)
	dispcm = ConfusionMatrixDisplay(confusion_matrix=male_cm, display_labels=target_names)
	dispcm.plot()
	dispcm.figure_.savefig('/MultiClassifier/Evaluation/'+outputinfo+'_fold'+str(fold_no)+'_maleCM.png') # Replace with actual file location
	...
	##### Export results
	# epoch history
	pd.DataFrame(history.history).to_csv('/MultiClassifier/History/'+outputinfo+'_fold'+str(fold_no)+'_history.csv') # Replace with actual file location
	
	# data to be written row-wise in csv file
	data =[["test loss", "test acc"], results]
	# opening the csv file in 'w+' mode
	file = open('/MultiClassifier/History/'+outputinfo+'_fold'+str(fold_no)+'_eval.csv', 'w+', newline ='') # Replace with actual file location
