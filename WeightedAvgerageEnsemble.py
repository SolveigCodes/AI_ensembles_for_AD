...
# Depends on the resulting models and stratlist (stratification file) from MultiClassifier_traincv.py
# Requires the balanced accuracy metric results (or other metric of choice) from MultiClassifier_traincv.py for use to rank base models
...
print(timestr)
# Script version
scriptversion = "Multi1TestWAvgEnsmblArchname" #Enter name of architecture of choice
print("Script: "+scriptversion)

# Common outputinfo
outputinfo = scriptversion+'_'+timestr+'_b'+str(batch_size)+'_e'+str(epochs)+'_archname_wavg_ensemble' # Adjust if wanted


##### Weighting strategy
# Enter chosen metric from trainingcv to determine weights for weighted average
# Example using balanced accuracy results
foldbaccs = [0.5265562024182714, 0.7293447293447293, 0.727688651218063, 0.5256485994190913, 0.5997737556561086] # Example results, replace with your own results

print("foldbaccs: ",foldbaccs)

# Determine weights for weighted average
# Ref. https://link.springer.com/article/10.1007/s41870-022-01149-8
# Iqball, T., Wani, M.A. Weighted ensemble model for image classification. 
# Int. j. inf. tecnol. 15, 557–564 (2023). https://doi.org/10.1007/s41870-022-01149-8

# metric from least accurate model
beta = min(foldbaccs)
# determine weights
wfac = [(x-beta+1) for x in foldbaccs]
print("weight factors: ", wfac)
rank = rankdata(foldbaccs)
rankfac = wfac*rank**2
wt = [x/sum(rankfac) for x in rankfac]
print("weights: ", wt)
...
data_path = '22-12-19/subjects/' # Replace with actual file location
...
# Paths to all models generated from 5-fold crossvalidation training that will be used inn a weighted average ensemble
path_model1 = '/Checkpoint/archname_fold1_best_model.hdf5'
print("path model 1: ",path_model1)
path_model2 = '/Checkpoint/archname_fold2_best_model.hdf5'
print("path model 2: ",path_model2)
path_model3 = '/Checkpoint/archname_fold3_best_model.hdf5'
print("path model 3: ",path_model3)
path_model4 = '/Checkpoint/archname_fold4_best_model.hdf5'
print("path model 4: ",path_model4)
path_model5 = '/Checkpoint/archname_fold5_best_model.hdf5'
print("path model 5: ",path_model5)
...
## Custom layer for ensemble with weighted average
## Ref. https://stackoverflow.com/questions/67647843/is-there-a-way-to-ensemble-two-keras-h5-models-trained-for-same-classes
...
# One part sent to cross validation
...
x_train_test.to_csv('/MultiClassifier/Temp/xtraintest_'+scriptversion+'_'+timestr+'.csv')
x_hold.to_csv('/MultiClassifier/Temp/xhold_'+scriptversion+'_'+timestr+'.csv')


# data sets outside cross validation
print('length of x_train_test: ', len(x_train_test))
...
#print("train batch size: ", batch_size, "test batch size: ", batch_size, "hold batch size: ", batch_size)
...
model1 = keras.models.load_model(path_model1, compile=False)
model1._name='archname_model1'
model2 = keras.models.load_model(path_model2, compile=False)
model2._name='archname_model2'
model3 = keras.models.load_model(path_model3, compile=False)
model3._name='archname_model3'
model4 = keras.models.load_model(path_model4, compile=False)
model4._name='archname_model4'
model5 = keras.models.load_model(path_model5, compile=False)
model5._name='archname_model5'

models = [model1, model2, model3, model4, model5]

model_input = layers.Input((cropdim, cropdim, cropdim, 1))
model_outputs = [model(model_input) for model in models]
ensemble_output = WeightedAverageLayer(wt[0],wt[1],wt[2],wt[3],wt[4])(model_outputs) # weighted average ensemble
...
ensemble_model.compile(loss=loss, optimizer=opt, metrics=['sparse_categorical_accuracy'])


##### Evaluate the model
results = ensemble_model.evaluate(hold_generator, verbose=2)
print("test loss: ",results[0])
print("test acc: ",results[1])

##### Model prediction
prediction = ensemble_model.predict(hold_generator, verbose=2)
...
r_results = ensemble_model.evaluate(remain_generator, verbose=2)
print("test loss remain: ",r_results[0])
print("test acc remain: ",r_results[1])

##### Model prediction
rprediction = ensemble_model.predict(remain_generator, verbose=2)
...
#print("length of remain_generator: ", len(remain_generator))
...
strattest = pd.DataFrame(list(zip(subject, test_labels, sx, education, mmse, ethn, test_predlabel, test_true, test_predprob)), columns = ['subj','diag_let','sex', 'edu', 'mmse', 'ethn', 'pred', 'true', 'prob'])
...
print("Completed: Weighted average ensemble with <archname>, epoch=200, outputinfo")
